...
export const initiateEsewaPayment = async (req, res) => {
    ...
    try {
        ...
        const msg = `total_amount=${total},transaction_uuid=${uuid},product_code=${ESEWA_CODE}`;
        const signature = crypto.createHmac('sha256', ESEWA_SECRET).update(msg).digest('base64');
        res.json({ success: true, enrollmentId: enrollment._id, esewaParams: { amount: String(total), tax_amount: "0", product_service_charge: "0", product_delivery_charge: "0", total_amount: String(total), transaction_uuid: uuid, product_code: ESEWA_CODE, success_url: `${FRONTEND_URL}/payment/success`, failure_url: `${FRONTEND_URL}/payment/failure`, signed_field_names: 'total_amount,transaction_uuid,product_code', signature }, url: 'https://rc-epay.esewa.com.np/api/epay/main/v2/form' });
    } catch (err) {
...
export const verifyKhaltiPayment = async (req, res) => {
    ...
    try {
        ...
        if (!enrollment) return res.status(404).json({ success: false, message: 'Enrollment not found' });
        const r = await fetch("https://a.khalti.com/api/v2/epayment/lookup/", { method: 'POST', headers: { 'Authorization': 'Key c9df6b5c328e44ebae5ccde01a6134a6', 'Content-Type': 'application/json' }, body: JSON.stringify({ pidx }) });
